...
######## FINAL PYTHON TRAINING EXERCISE
...
# Main function that runs the app is = main() function at the bottom of this file
...
def main():
    ...
    datasetpath = r'C:\Users\teresa.diaz.calvo\source\repos\trainingpython\Data_ejercicio_final_python_training\historico_llamadas.xlsx'
    
    #df = pd.read_csv(contrato_factura_datasetpath, sep=';', decimal='.')
    df = pd.read_excel(datasetpath, sheet_name='Sheet1', decimal='.')

    conditions = [
        (df['FacturaciÃ³n'] > float(4000)), #FacturaciÃ³n
        ((df['FacturaciÃ³n'] >= float(3000)) & (df['FacturaciÃ³n'] < float(4000))),
        (df['FacturaciÃ³n'] < float(3000))
        ]
    ...
    df1 = df1.rename(columns={'FacturaciÃ³n':'Calls'})
    ...
    df2 = df2.rename(columns={'FacturaciÃ³n':'Calls'})
    ...
    df3 = df3.rename(columns={'FacturaciÃ³n':'Calls'})
    ...
    df4 = df4.rename(columns={'FacturaciÃ³n':'Calls'})
