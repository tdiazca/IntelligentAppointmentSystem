...
def main():
    ...
    conditions = [
        (df['Facturaci贸n'] > float(4000)), #Facturaci贸n
        ((df['Facturaci贸n'] >= float(3000)) & (df['Facturaci贸n'] < float(4000))),
        (df['Facturaci贸n'] < float(3000))
        ]
    ...
    ## Genera la clase: Predicci髇() which has a method to calculate the standard deviation and the mean, another to predict with that standard deviation and mean,
    ...
    df1 = df1.rename(columns={'Facturaci贸n':'Calls'})
    ...
    df2 = df2.rename(columns={'Facturaci贸n':'Calls'})
    ...
    df3 = df3.rename(columns={'Facturaci贸n':'Calls'})
    ...
    df4 = df4.rename(columns={'Facturaci贸n':'Calls'})
...
## Predicci髇() - OJO, normal distribution with those std gives some negative numbers for predicted calls! - some reserved slots become a negative number!
